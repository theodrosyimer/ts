...
type Test00<T1 extends any[], T2 extends any[]> = Reverse<
  Cast<Reverse<T1>, any[]>,
  T2
>
...
type CurryV3<P extends any[], R> = <T extends any[]>(
  ...
  Drop<Length<T>, P> extends infer DT ? Cast<DT, any[]> : never
> extends 0
  ? R
...
const test49 = curried09('Jane')(26)(true, 'JJ', 900000)
...
> = {
  0: GapsOf<
    T1,
    T2,
    GapOf<T1, T2, TN, I> extends infer G ? Cast<G, any[]> : never,
    Next<I>
  >
...
const toCurry12 = (name: string, surname: string, age: number, single: true) =>
  true
